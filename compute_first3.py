...
# 指定Excel文件路径
excel_file_path = 'first3/CK-1.xlsx'
excel_file_path1 = 'first3/Dtu-1.xlsx'
...
CK_Actin, gene1, gene2, gene3, chuli_Actin, genechuli1, genechuli2, genechuli3 = random(df, df1,
        'CK-Actin', 'CK-OsLox2', 'CK-OsPBZ1', 'CK-OsWRKY70', 'Dtu-Actin', 'Dtu-OsLox2', 'Dtu-OsPBZ1', 'Dtu-OsWRKY70', the_time)
...
# 创建一个 DataFrame
# 创建一个 DataFrame
columns = [f'OsLox2_{i+1}' for i in range(0, result_shape[0] + 1)] + \
          [f'OSPBZ1_{i+1}' for i in range(0, result_shape[0] + 1)] + \
          [f'OsWRKY70_{i+1}' for i in range(0, result_shape[0] + 1)]
results_df = pd.DataFrame(np.concatenate(all_results_list, axis=1), columns=columns)

# 将结果保存到 Excel 文件
results_df.to_excel(f'{the_time}_results.xlsx', index=False)
